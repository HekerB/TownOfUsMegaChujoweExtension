...
public static class MirageDecoySystem
{
    ...
    public static void PrimeDecoy(byte mirageId, PlayerControl appearanceSource, Vector3 worldPos, float zRot, bool flipX)
    {
        ...
        fake.body.transform.rotation = Quaternion.Euler(0, 0, zRot);
        if (flipX)
        {
            fake.body.transform.localScale = new Vector3(-1, 1, 1);
        }

        CopyPet(appearanceSource, fake.body);
    ...
    private static void CopyPet(PlayerControl source, GameObject targetBody)
    {
        if (source == null || source.cosmetics == null || source.cosmetics.currentPet == null) return;

        var sourcePet = source.cosmetics.currentPet;
        var decoyPet = UnityEngine.Object.Instantiate(sourcePet.gameObject, targetBody.transform);
        decoyPet.name = "DecoyPet";

        // Offset relative to body
        decoyPet.transform.localPosition = new Vector3(-0.3f, -0.2f, -0.01f);

        var pb = decoyPet.GetComponent<PetBehaviour>();
        if (pb != null)
        {
            pb.enabled = false;
        }
