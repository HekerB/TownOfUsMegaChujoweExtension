...
public static class PelicanSystem
{
    ...
    public static void DigestAll(byte pelicanId)
    {
        ...
        foreach (var victimId in victims.ToList())
        {
            ...
            if (victim != null && !victim.HasDied())
            {
                ...
                {
                    try { victim.AddModifier<DeathHandlerModifier>(); } catch { /* ignore */ }
                }
    ...
    private static Vector2 FindSafePosition(Vector2 targetPosition)
    {
        ...
        {
            return OriginalPositions.Values.FirstOrDefault(pos => IsPositionSafe(pos, pos));
        }
        ...
        var firstSafePos = OriginalPositions.Values.FirstOrDefault(pos => IsPositionSafe(pos, pos));
        if (firstSafePos != default) return firstSafePos;
    ...
    public static void ShowSwallowedNotification()
    {
        ...
        try
        {
            ...
            if (notif != null)
            {
                ...
                try { notif.AdjustNotification(); } catch { /* ignore */ }
                try
                {
    ...
    public static void ForceResetAllPlayers()
    {
        ...
        foreach (var player in PlayerControl.AllPlayerControls)
        {
            ...
            try { RemoveSwallowedModifier(player); } catch { /* ignore */ }
            player.Visible = true;
            player.moveable = true;
            RestoreFootstepsIfNeeded(player);
            try { player.NetTransform.Halt(); } catch { /* ignore */ }
        }
