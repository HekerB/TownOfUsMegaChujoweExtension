...
public static class ExtensionLocale
{
    ...
    private static void ForceInjectTranslations(Stream stream)
    {
                ...
                foreach (XmlNode node in nodes)
                {
                    ...
                    if (!string.IsNullOrEmpty(key))
                    {
                        foreach (var langKey in TouLocale.LangList.Keys)
                        {
                            ...
                            {
                                TouLocale.TouLocalization[langEnum] = dict = new System.Collections.Generic.Dictionary<string, string>();
                            }
        ...
        {
            System.Console.WriteLine($"[ExtensionLocale] Error injecting translations: {ex}");
        }
    }
}
