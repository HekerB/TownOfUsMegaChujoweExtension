...
public static class DraftSystem
{
    ...
    public static HashSet<byte> LastNeutralKillingIds { get; set; } = new();
    ...
    private static List<T> WeightedShuffle<T>(IEnumerable<T> items, System.Func<T, float> weightSelector)
    {
    ...
    private static IEnumerable<RoleBehaviour> OrderRoles(IEnumerable<RoleBehaviour> roles)
    {
        ...
        return WeightedShuffle(roles.ToList(), r => Mathf.Max(0.1f, MiscUtils.GetAssignData(r.Role).Chance));
    }
