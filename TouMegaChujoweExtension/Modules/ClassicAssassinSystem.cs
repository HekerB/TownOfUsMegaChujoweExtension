...
public static class ClassicAssassinSystem
{
    ...
    private static void BuildGuessableListForVigilante()
    {
        ...
        var roles = GetGuessableRolesWithDynamicSupport(
            MiscUtils.GetPotentialRoles().Where(r => IsRoleValid(r))
        );
    ...
    private static List<BaseModifier> GetGuessableModifiersWithDynamicSupport(IEnumerable<BaseModifier> baseModifiers)
    {
        ...
        try
        {
            ...
            foreach (var type in extensionTypes.Where(t => !modifiers.Any(m => m.GetType() == t)))
            {
                try
                {
    ...
    public static void RefreshExemptions()
    {
        ...
        if (IsActive)
        {
            ...
            foreach (var targetId in Buttons.Keys.ToList())
            {
                ...
                if (local.TryGetModifier<AssassinModifier>(out var assassin) && IsExempt(voteArea, assassin))
                {
                    HideSingle(targetId);
                }
