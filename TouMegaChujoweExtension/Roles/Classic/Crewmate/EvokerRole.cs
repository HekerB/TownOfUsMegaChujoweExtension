...
public sealed class EvokerRole(IntPtr cppPtr) : CrewmateRole(cppPtr), ITownOfUsRole, IWikiDiscoverable, IDoomable
{
    ...
    public List<string> VerifiedRecords = new();
    ...
    public override void Initialize(PlayerControl player)
    {
        ...
        VerifiesUsed = 0;
        VerifiedRecords = new List<string>();
...
public static void RpcEvokerBlind(PlayerControl evoker, float duration)
{
    ...
    public StringBuilder SetTabText()
    {
        ...
        if (VerifiedRecords.Count != 0)
        {
            stringB.AppendLine($"\n<b>{TouLocale.Get("ExtensionRoleEvokerTabHeader", "Verified Players:")}</b>");
            foreach (var record in VerifiedRecords)
            {
                stringB.AppendLine($"<b><size=70%>{record}</size></b>");
            }
        }
