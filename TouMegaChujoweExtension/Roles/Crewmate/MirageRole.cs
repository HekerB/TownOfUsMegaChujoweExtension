...
public sealed class MirageRole(IntPtr cppPtr) : CrewmateRole(cppPtr), ITownOfUsRole, IWikiDiscoverable, IDoomable, IUnguessable
{
    ...
    internal static readonly System.Collections.Generic.Dictionary<byte, System.Collections.Generic.List<string>> TriggeredRoles = new();
    ...
    public static void RpcMirageTriggerDecoy(PlayerControl mirage, PlayerControl interactor, Vector2 pos)
    {
        ...
        {
            if (!TriggeredRoles.TryGetValue(mirage.PlayerId, out var triggeredList))
            {
                triggeredList = new System.Collections.Generic.List<string>();
                TriggeredRoles[mirage.PlayerId] = triggeredList;
            }
            ...
            if (!triggeredList.Contains(coloredRoleName))
            {
                triggeredList.Add(coloredRoleName);
            }
        }
