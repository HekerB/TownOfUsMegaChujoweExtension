...
public static class InjectorEvents
{
    ...
    public static void ScheduleInjection(PlayerControl injector, PlayerControl target)
    {
        ...
        if (!PendingInjections.TryGetValue(target.PlayerId, out var pendingList))
        {
            pendingList = new List<PendingInjection>();
            PendingInjections[target.PlayerId] = pendingList;
        }
    ...
    private static IEnumerator CoApplyInjection(PendingInjection pending)
    {
        ...
        {
            if (pending.Target != null && PendingInjections.TryGetValue(pending.Target.PlayerId, out var pendingList))
            {
                pendingList.RemoveAll(p => p.InjectionId == pending.InjectionId);
        ...
        if (PendingInjections.TryGetValue(pending.Target.PlayerId, out var finalPendingList))
        {
            finalPendingList.RemoveAll(p => p.InjectionId == pending.InjectionId);
            if (finalPendingList.Count == 0)
            {
                PendingInjections.Remove(pending.Target.PlayerId);
            }
        }
    ...
    private static void ApplyInjectionEffect(PlayerControl injector, PlayerControl target, Guid injectionId)
    {
        ...
        AppliedInjectionCounts.TryGetValue(target.PlayerId, out var count);
        count++;
        AppliedInjectionCounts[target.PlayerId] = count;
    ...
    public static void EjectionEventHandler(EjectionEvent @event)
    {
        ...
        var keysToCheck = PendingInjections.Keys.Where(k => PendingInjections[k].Any(p => p.Injector?.PlayerId == exiled.PlayerId)).ToList();
        ...
        // Removed empty loop that did nothing
    }
