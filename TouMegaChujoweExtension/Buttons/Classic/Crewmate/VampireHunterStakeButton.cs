...
public sealed class StakeButton : TownOfUsRoleButton<VampireHunterRole>, IKillButton
{
    ...
    protected override void OnClick()
    {
        ...
        var isVampire = _target.IsRole<VampireRole>();
        if (isVampire)
        {
            if (role.SuccessfulStakes == 0)
            {
                role.SuccessfulStakes++;
            }
            
            if (!HunterKilledVictims.Contains(_target.PlayerId))
            {
                HunterKilledVictims.Add(_target.PlayerId);
            }

            player.RpcSpecialMurder(_target, causeOfDeath: "ExtensionRoleVampireHunterStaked");

            if (player.AmOwner)
            {
                Helpers.CreateAndShowNotification(
                    TouLocale.Get("DiedToExtensionRoleVampireHunterStaked"),
                    Color.white,
                    new Vector3(0f, 1f, -20f),
                    spr: TouExtensionIcons.VampireHunterRoleIcon.LoadAsset())?.AdjustNotification();
            }
        }
        else
        {
