...
public sealed class VanisherVanishButton : TownOfUsRoleButton<VanisherRole>
{
    ...
    public override bool CanUse()
    {
        ...
        bool hasUses = !LimitedUses || UsesLeft > 0;
        if (!hasUses && !EffectActive) return false;

        // Highlight if enemy nearby, timer up, or already active
        return (HasEnemyNearby() || Timer <= 0 || EffectActive);
    }
    ...
    private bool HasEnemyNearby()
    {
        ...
        var options = OptionGroupSingleton<VanisherOptions>.Instance;
        if (!options.DetectionEnabled) return false;

        float radius = options.DetectionRadius.Value;
        var myPos = player.GetTruePosition();

        foreach (var other in PlayerControl.AllPlayerControls)
        {
            if (other == null || other.PlayerId == player.PlayerId || other.HasDied() || other.Data == null || other.Data.Disconnected)
                continue;

            if (other.IsImpostor() || other.IsNeutral())
            {
                if (Vector2.Distance(myPos, other.GetTruePosition()) <= radius)
                    return true;
            }
        }
        return false;
