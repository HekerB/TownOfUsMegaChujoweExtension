...

{
    ...
    private static readonly List<byte> _hunterKilledVictims = new();
    public static bool HasHunterKilledVictim(byte id) => _hunterKilledVictims.Contains(id);
    public static void AddHunterKilledVictim(byte id) => _hunterKilledVictims.Add(id);
    public static void ClearHunterKilledVictims() => _hunterKilledVictims.Clear();
    ...
    protected override void OnClick()
    {
        ...
        if (isVampire)
        {
            player.RpcSpecialMurder(_target, causeOfDeath: "Staked");
            role.SuccessfulStakes++;

            if (!OptionGroupSingleton<VampireHunterOptions>.Instance.CanSelfReport)
            {
                AddHunterKilledVictim(_target.PlayerId);
            }
        }
        else
        {
