...
public static class DraftLobbyPatch
{
    ...
    // === STATE ===
    public static bool DraftInProgress { get; set; }
    private static float _pickTimer;
    private static bool _countdownWasActive;
    public static bool DraftCompletedWaitingForStart { get; set; }
...
#pragma warning disable CS0414, S4487
    private static bool _alertPlayed;
    private static int _currentDangerLevel = -1;
    private static bool _clipsLoading;
#pragma warning restore CS0414, S4487
    ...
    {
        if (!DraftInProgress && !DraftCompletedWaitingForStart) return;
    ...
    public static void GameStartManagerStart()
    {
        ...
        DraftCompletedWaitingForStart = false;
        _alertPlayed = false;
        _lobbyLocked = false;
    ...
    private static System.Collections.IEnumerator CoPreloadHnsSounds()
    {
        ...
        foreach (var address in allNames)
        {
            ...
            {
                // Ignore missing clips in Addressables
            }
        }
    ...
    public static void GameStartManagerUpdate(GameStartManager __instance)
    {
        ...
        if (DraftCompletedWaitingForStart)
        {
            UpdateDraftUI(__instance);
            return;
        }

        if (__instance.startState == GameStartManager.StartingStates.Countdown && !DraftInProgress && !_countdownWasActive)
            _countdownWasActive = true;

        if (_countdownWasActive && !DraftInProgress && __instance.startState == GameStartManager.StartingStates.Countdown)
        {
        ...
        if (DraftInProgress) UpdateDraftUI(__instance);
    }
    ...
    {
        if (AmongUsClient.Instance.AmHost) return;
        if (DraftCompletedWaitingForStart) { UpdateDraftUI(__instance); return; }
        if (DraftInProgress) UpdateDraftUI(__instance);
    }
    ...
    public static bool BeginGamePrefix()
    {
        ...
        if (DraftCompletedWaitingForStart) return true;
        return true;
    }
    ...
    {
        if (DraftSystem.IsEnabled && DraftCompletedWaitingForStart)
        {
...
#pragma warning restore S3011
            ...
            {
                // Ignore reflection errors if TownOfUs version changes
            }
    ...
    private static System.Collections.IEnumerator CoLoadDangerClipsAndPlay()
    {
        ...
        foreach (var address in namesToTry)
        {
            ...
            {
                // Ignore missing clip from Addressables
            }
        }
    ...
    private static void StartMusicWithClips()
    {
        ...
        try
        {
            ...
            var totalDraftPickers = _originalPickOrder.Count;
    ...
    private static void CrossfadeToClip(AudioClip newClip)
    {
        ...
        try { next.Stop(); } catch { /* ignore stop error */ }
        ...
        try { next.time = syncTime; } catch { /* ignore seek error */ }
        next.Play();
    ...
    private static void StopDraftMusic()
    {
        if (_draftMusicSourceA != null || _draftMusicSourceB != null)
        {
            ...
            try { if (_draftMusicSourceA != null) { _draftMusicSourceA.Stop(); _draftMusicSourceA.clip = null; } } catch { /* ignore */ }
            try { if (_draftMusicSourceB != null) { _draftMusicSourceB.Stop(); _draftMusicSourceB.clip = null; } } catch { /* ignore */ }
    ...
    private static void PlayDraftCompleteSound()
    {
        ...
        if (clip != null)
        {
            ...
            {
                // Ignore missing sound or sound manager errors
            }
        }
    ...
    private static void CreateDraftUI()
    {
        ...
        try { SoundManager.Instance.StopAllSound(); } catch { /* Ignore sound stopping errors */ }
    ...
    private static string? GetRoleDescription(RoleBehaviour? role)
    {
        ...
        {
            // Ignore reflection errors for custom roles
        }
        return null;
    }
    ...
    private static System.Collections.IEnumerator CoFadeOutRoleButtons(float duration = 1.0f)
    {
        ...
        foreach (var data in buttonData.Where(d => d.obj != null))
    ...
    private static void CreateCancelButton()
    {
        ...
        bgRenderer.sortingOrder = 15;
        bgObj.transform.localScale = new Vector3(0.24f, 0.19f, 1f); // krÄ‚Å‚tszy
        ...
        collider.size = new Vector2(0.74f, 0.36f); // krÄ‚Å‚tszy hitbox
    ...
    private static System.Collections.IEnumerator CoAnimateSelection(GameObject? selected)
    {
        ...
        {
            selectedOrigBG = selectedRefs!.BG?.color ?? Color.white;
        }
    ...
    private static System.Collections.IEnumerator CoAnimateButtonHover(GameObject obj, float targetScale, float duration)
    {
        ...
        if (obj != null && _buttonRefs.TryGetValue(obj, out var refs)) border = refs.Border;
    ...
    private static Sprite? GetRoleIcon(RoleBehaviour? role)
    {
        ...
        {
            // Ignore missing icon or reflection errors
        }
        return null;
    }
    ...
    private static void CreateRoleButton(float x, float y, RoleBehaviour? role, bool isRandom, bool isImp)
    {
        ...
        if (isRandom)
        {
            ...
            {
                // Ignore missing random icon to avoid stopping button creation
            }
        }
        ...
        label.text = isRandom ? "RANDOM" : role!.GetRoleName().ToUpperInvariant();
        label.fontSize = 2.2f;
    ...
    {
        try { SoundManager.Instance.PlaySound(TouExtensionAudio.DraftPickSound.LoadAsset(), false); } catch { /* Ignore sound errors */ }
    }
    ...
    private static void OnDraftComplete()
    {
        PlayPickSound();
        DraftInProgress = false;
        DraftCompletedWaitingForStart = true;
    ...
    private static System.Collections.IEnumerator CoAnimateDraftTitle()
    {
        ...
        float interval = 20f;
        while (DraftInProgress || DraftCompletedWaitingForStart)
        {
    ...
    private static void UpdateDraftUI(GameStartManager gsm)
    {
        ...
        if (DraftSystem.IsMyTurn && timeLeft <= 11f && timeLeft > 0f)
        {
            ...
            if (_countdownSoundTimer <= 0f)
            {
                ...
                {
                    // Ignore missing sound or sound manager errors to prevent breaking the loop
                }
        ...
        if (DraftSystem.IsMyTurn && !_pickLocked && _pickTimer >= maxTime)
        {
            ...
            var localId = localPlayer.PlayerId;
    ...
    public static void OnPlayerLeft(AmongUsClient __instance, [HarmonyArgument(0)] InnerNet.ClientData client, [HarmonyArgument(1)] DisconnectReasons reason)
    {
        ...
        if (!DraftInProgress && DraftCompletedWaitingForStart)
        {
            return;
        }
    ...
    {
        if (DraftInProgress && Input.GetKeyDown(KeyCode.Return) && !__instance.IsOpenOrOpening)
        {
