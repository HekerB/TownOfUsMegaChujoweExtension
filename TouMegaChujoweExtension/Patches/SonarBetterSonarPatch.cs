...
{
    public static Dictionary<byte, GameObject> TrackerIcons { get; } = new();
        ...
        try
        {
            ...
            foreach (var assembly in assemblies)
            {
                ...
                if (type != null)
                {
                    ...
                    {
                        var props = type.GetProperties(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance);
                        foreach (var prop in props.Where(p => p.Name.Contains("Delay", System.StringComparison.OrdinalIgnoreCase) || p.Name.Contains("Interval", System.StringComparison.OrdinalIgnoreCase)))
                        {
                            var val = prop.GetValue(instance);
    ...
    public static void RoundStartEventHandler(RoundStartEvent @event)
    {
        ...
        foreach (var tracker in localTrackers.Where(t => t != null))
        {
    ...
    {
        foreach (var icon in TrackerIcons.Values.Where(i => i != null))
        {
            Object.Destroy(icon);
        }
        TrackerIcons.Clear();
