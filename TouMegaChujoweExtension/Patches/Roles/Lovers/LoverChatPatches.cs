...
public static class LoverChatPatches
{
    ...
    public static bool SendChatPatch(ChatController __instance)
    {
        ...
        // Pelican block: Swallowed players cannot use Lover Chat
        if (PlayerControl.LocalPlayer.HasModifier<PelicanSwallowedModifier>())
        {
            return true;
        }
        ...
        if (PlayerControl.LocalPlayer.HasModifier<LoverModifier>())
        {
            ...
            {
                // Note: AddTeamChat might be in TownOfUs.Utilities.MiscUtils
                TownOfUs.Utilities.MiscUtils.AddTeamChat(PlayerControl.LocalPlayer.Data,
