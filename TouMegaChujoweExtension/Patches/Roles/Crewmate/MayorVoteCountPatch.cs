    ...
    public static void PostfixOnMeetingStart(MayorRole __instance)
    {
        ...
        if (voteData != null)
        {
            ...
            foreach (var name in names)
            {
                ...
                if (field != null) try { field.SetValue(__instance, count); } catch { /* ignore field sync error */ }
                
                var prop = AccessTools.Property(typeof(MayorRole), name);
                if (prop != null && prop.CanWrite) try { prop.SetValue(__instance, count); } catch { /* ignore prop sync error */ }
            }
        }
    }
