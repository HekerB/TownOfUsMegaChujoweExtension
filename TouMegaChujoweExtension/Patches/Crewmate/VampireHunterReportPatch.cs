...
public static class VampireHunterReportPatch
{
    ...
    private static DeadBody? GetClosestReportableBody(PlayerControl player)
    {
        ...
        foreach (var body in UnityEngine.Object.FindObjectsOfType<DeadBody>())
        {
            ...
            if (distance < closestDistance && 
                (distance < 0.5f || !PhysicsHelpers.AnythingBetween(playerPos, body.TruePosition, Constants.ShipAndObjectsMask, false)))
            {
    ...
    public static void HudManagerUpdatePostfix(HudManager __instance)
    {
        ...
        if (body != null)
        {
            if (StakeButton.HasHunterKilledVictim(body.ParentId))
            {
    ...
    public static bool DoClickPrefix()
    {
        ...
        var body = GetClosestReportableBody(local);
        if (body != null && StakeButton.HasHunterKilledVictim(body.ParentId))
        {
            return false;
        }
        return true;
    ...
    public static bool DeadBodyOnClickPrefix(DeadBody __instance)
    {
        ...
        if (StakeButton.HasHunterKilledVictim(__instance.ParentId))
        {
            return false;
        }
        return true;
