...
public static class ExtensionSymbolsPatch
{
    ...
    public static void UpdateTargetSymbolsPostfix(ref string __result, PlayerControl player, bool hidden)
    {
        ...
        // --- POPE (Θ) ---
        if (player.HasModifier<PopeCanonizedModifier>() && !__result.Contains('Θ') &&
            (local.Data.Role is PopeRole || deadKnow))
        {
            __result += " <color=#FFD700>Θ</color>";
        }

        // --- DEATH NOTE (♡) ---
        if (TryGetDeathNoteTarget(out var dnTarget) && dnTarget != null && dnTarget.PlayerId == player.PlayerId && !__result.Contains('♡') &&
            (local.HasModifier<DeathNoteModifier>() || deadKnow))
        {
            __result += " <color=#8B00FF>♡</color>";
        }

        // --- SHROUD (♢) ---
        if (player.TryGetModifier<ShroudedModifier>(out var shroudMod) && !__result.Contains('♢') &&
            ((local.IsRole<ShroudRole>() && shroudMod.ShroudOwnerId == local.PlayerId) || deadKnow))
        {
