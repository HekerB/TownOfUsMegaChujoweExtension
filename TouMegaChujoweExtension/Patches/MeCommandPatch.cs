...
public static class MeCommandPatch
{
    ...
    public static bool Prefix(ChatController __instance)
    {
        ...
        var spaceLess = text.Replace(" ", "").ToLowerInvariant();
        if (spaceLess.StartsWith("/draft", StringComparison.Ordinal))
        {
            HandleDraftCommand(__instance, text);
            return false;
        }

        if (spaceLess.StartsWith("/setname", StringComparison.Ordinal) && (DraftLobbyPatch.DraftInProgress || DraftLobbyPatch.DraftCompletedWaitingForStart))
        {
        ...
        if (!spaceLess.StartsWith("/me", StringComparison.Ordinal)) return true;
    ...
    private static string BuildPlayerInfo(PlayerControl player, List<ModLinkInfo> modLinks)
    {
        ...
        if (!LobbyBehaviour.Instance && role != null && role.TeamType != RoleTeamTypes.Impostor)
        {
            try
            {
                ...
                if (total > 0)
                    sb.AppendLine("Tasks: " + completed.ToString(System.Globalization.CultureInfo.InvariantCulture) + "/" + total.ToString(System.Globalization.CultureInfo.InvariantCulture));
            }
            catch
            {
                // Ignore task collection errors
            }
        }
