...
public static class PirateDuelMeetingPatch
{
    ...
    {
        var area = meetingHud.playerStates.FirstOrDefault(a => a != null && a.TargetPlayerId == targetId);
        if (area == null || area.NameText == null) return;
        ...
        if (!area.NameText.text.StartsWith("☠ ", System.StringComparison.Ordinal))
        {
    ...
    private static void CreateDuelButton(MeetingHud meetingHud)
    {
        ...
        PlayerVoteArea? myVoteArea = meetingHud.playerStates.FirstOrDefault(pva => pva.TargetPlayerId == localId);
